...
# train_data.to_csv("train_data2.txt", mode='w', index = False, header = False)
...
print(len(train_data))  # 2354 에서 2142개로
...
print('전처리 후 테스트용 샘플의 개수 :',len(test_data)) # 1009 에서 957

# train_data['title'].to_csv("cleandata.txt", mode='w', index = False, header = False)
#############################
############ 토큰화 ############
#############################
# 불용어 정의
stopwords_read = pd.read_csv("stopwords_self.txt", sep="\n", header=None)
...
model2.save('model2.h5')     # 위에 작업을 한번만 하기위해 저장
'''
